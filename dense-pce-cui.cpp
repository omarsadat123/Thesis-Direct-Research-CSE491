...
static bool g_enable_turan       = true; // mode 4
static bool g_enable_uncle_inh   = false; // mode 5
static bool g_enable_outside_gap = false; // mode 6
static bool g_enable_structural_child = false; // mode 7/8

#ifndef DENSE_PCE_CHECK_DPSI
#ifndef NDEBUG
#define DENSE_PCE_CHECK_DPSI 1
#else
#define DENSE_PCE_CHECK_DPSI 0
#endif
#endif
...
private:
    ...
    // --- CUI depth-indexed scratch buffers ---
    // dpsi_stack[k]     : the full ψ-feasible pool D_ψ(K) computed while |P|==k
    // children_stack[k] : the children list derived at |P|==k
    // Pre-allocated once in the constructor; cleared (not reallocated) per call.
    // Safety: iter() at depth k only reads dpsi_stack[k-1] (parent) and writes
    // dpsi_stack[k] and children_stack[k]. Recursive calls touch only depth k+1
    // and deeper, so sibling iteration at depth k sees stable buffers.
    std::vector<std::vector<int>> dpsi_stack;
    std::vector<std::vector<int>> children_stack;
    int cui_pool_start_depth = 2; // enable pool maintenance from this |P| depth
    static constexpr size_t kUncleInheritVetoFactor = 4;
    ...
    unsigned long long numcalls_saved_by_uncle_inh = 0ULL;
    unsigned long long num_outside_gap_fastpath_hits = 0ULL;
    unsigned long long num_outside_gap_candidates    = 0ULL;

    // Structural-child accounting
    unsigned long long num_structural_A_singleton_nodes           = 0ULL;
    unsigned long long num_structural_A_singleton_B_empty_nodes   = 0ULL;
    mutable unsigned long long num_structural_type2_fastpath_hits = 0ULL;
    mutable unsigned long long num_structural_type3_partial_uses  = 0ULL;
    mutable unsigned long long num_structural_type3_full_hits     = 0ULL;

    inline bool outside_gap_holds(int deltaP) const {
        // M(K) < δ(K)
        return (deltaP > 0) && (maxNPdeg < deltaP);
    }

    inline bool p_bucket_singleton(int deg) const {
        return deg >= 0 &&
               deg < static_cast<int>(countP.size()) &&
               countP[deg] == 1;
    }

    inline bool p_bucket_empty(int deg) const {
        return deg < 0 ||
               deg >= static_cast<int>(countP.size()) ||
               countP[deg] == 0;
    }

    void collect_full_dpsi_from_np(int min_add_deg, std::vector<int>& out) const;
    void collect_children_by_narrow_scan(int v_star, int min_add_deg, int deltaP,
                                         std::vector<int>& out) const;
    bool is_child_by_lemma5_ii_iii(int u, int v_star, int deltaP) const;
#if DENSE_PCE_CHECK_DPSI
    void debug_assert_dpsi_matches_current(int min_add_deg,
                                           const std::vector<int>& child_dpsi) const;
#endif
...
public:
    PseudoCliqueEnumerator(Graph& graph, float theta = 1.0, int min_size = 1, int max_size = -1, int order_ub = -1) : graph(graph), theta(theta), min_size(min_size) {        
        ...
        // CUI scratch buffers: one per recursion depth, pre-reserved.
        // max_size+2 slots covers depths 0..max_size and a guard for k+1 access.
        dpsi_stack.assign(this->max_size + 2, std::vector<int>{});
        children_stack.assign(this->max_size + 2, std::vector<int>{});
        for (auto& b : dpsi_stack)     b.reserve(64);
        for (auto& b : children_stack) b.reserve(64);

        // Earliest depth where theta * k can even reach 1.
        // Before this depth, inheritance cannot pass theta*k >= delta+1.
        cui_pool_start_depth = std::max(1, static_cast<int>(std::ceil(1.0 / static_cast<double>(theta))));
    }
    ...
    unsigned long long get_numcalls_saved_by_uncle_inh() const {
        return numcalls_saved_by_uncle_inh;
    }

    unsigned long long get_num_outside_gap_fastpath_hits() const {
        return num_outside_gap_fastpath_hits;
    }

    unsigned long long get_num_outside_gap_candidates() const {
        return num_outside_gap_candidates;
    }

    unsigned long long get_num_structural_A_singleton_nodes() const {
        return num_structural_A_singleton_nodes;
    }

    unsigned long long get_num_structural_A_singleton_B_empty_nodes() const {
        return num_structural_A_singleton_B_empty_nodes;
    }

    unsigned long long get_num_structural_type2_fastpath_hits() const {
        return num_structural_type2_fastpath_hits;
    }

    unsigned long long get_num_structural_type3_partial_uses() const {
        return num_structural_type3_partial_uses;
    }

    unsigned long long get_num_structural_type3_full_hits() const {
        return num_structural_type3_full_hits;
    }

    unsigned long long get_num_structural_total_candidate_hits() const {
        return num_structural_type2_fastpath_hits
             + num_structural_type3_partial_uses
             + num_structural_type3_full_hits;
    }
...
void PseudoCliqueEnumerator::collect_full_dpsi_from_np(int min_add_deg,
                                                       std::vector<int>& out) const {
    out.clear();
    const int start_deg = std::max(0, min_add_deg);
    if (start_deg > maxNPdeg) return;

    for (int deg = start_deg; deg <= maxNPdeg; ++deg) {
        for (int u = headNP[deg]; u != -1; u = nextNP[u]) {
            if (tracks[u].inP) continue;
            if (static_cast<int>(tracks[u].degNP) >= start_deg) {
                out.push_back(u);
            }
        }
    }
...
void PseudoCliqueEnumerator::collect_children_by_narrow_scan(int v_star,
                                                             int min_add_deg,
                                                             int deltaP,
                                                             std::vector<int>& out) const {
    out.clear();

    // Type 1: deg in [min_add_deg, deltaP-1]
    ...
    for (int u = headNP[deltaP]; u != -1; u = nextNP[u]) {
        ...
        if (u < v_star) {
            out.push_back(u);
        } else if (graph.has_edge(v_star, u)) {
            bool valid = true;
            const bool type2_fast =
                g_enable_structural_child && p_bucket_singleton(deltaP);

            if (!type2_fast) {
                for (int x = headP[deltaP]; x != -1; x = nextP[x]) {
                    // When structural-child is enabled, v_star was already checked.
                    if (g_enable_structural_child && x == v_star) continue;
            ...
            if (valid) {
                if (type2_fast) num_structural_type2_fastpath_hits++;
                out.push_back(u);
            }
        }
    }
    ...
    if (targetDeg < static_cast<int>(headNP.size())) {
        for (int u = headNP[targetDeg]; u != -1; u = nextNP[u]) {
            if (tracks[u].degNP < min_add_deg) continue;
            if (!graph.has_edge(v_star, u) || !(v_star > u)) continue;

            bool valid = true;

            const bool a_singleton =
                g_enable_structural_child && p_bucket_singleton(deltaP);
            const bool b_empty =
                g_enable_structural_child && p_bucket_empty(targetDeg);

            // Need A(K) walk only if A is not singleton.
            if (!a_singleton) {
                for (int x = headP[deltaP]; x != -1; x = nextP[x]) {
                    // When structural-child is enabled, v_star was already checked.
                    if (g_enable_structural_child && x == v_star) continue;
            ...
            // Need B(K) walk unless full fast path applies.
            if (valid && !b_empty && targetDeg < static_cast<int>(headP.size())) {
            ...
            if (valid) out.push_back(u);
        }
    }
}

bool PseudoCliqueEnumerator::is_child_by_lemma5_ii_iii(int u,
                                                       int v_star,
                                                       int deltaP) const {
    const int du = static_cast<int>(tracks[u].degNP);
    ...
    if (du == deltaP) {
        if (u < v_star) return true;
        if (!graph.has_edge(v_star, u)) return false;
    ...
    if (du == deltaP + 1) {
        if (!graph.has_edge(v_star, u) || !(v_star > u)) return false;
        ...
        if (!b_empty && targetDeg < static_cast<int>(headP.size())) {
            for (int x = headP[targetDeg]; x != -1; x = nextP[x]) {
                if (x < u && !graph.has_edge(u, x)) return false;
            }
        }
...
#if DENSE_PCE_CHECK_DPSI
void PseudoCliqueEnumerator::debug_assert_dpsi_matches_current(
    int min_add_deg,
    const std::vector<int>& child_dpsi) const {

    std::vector<int> rebuilt;
    collect_full_dpsi_from_np(min_add_deg, rebuilt);

    std::vector<int> lhs = child_dpsi;
    std::sort(lhs.begin(), lhs.end());
    std::sort(rebuilt.begin(), rebuilt.end());

    if (lhs != rebuilt) {
        std::cerr << "[D_PSI invariant failed] |P|=" << total_nodes_in_P
                  << " min_add_deg=" << min_add_deg
                  << " delta(P)=" << min_degree_in_P_fast()
                  << " cached=" << lhs.size()
...
void PseudoCliqueEnumerator::iter(int v) {
    ...
    const int min_add_deg = std::max(0, static_cast<int>(std::ceil(theta_P)));
    const int deltaP = min_degree_in_P_fast();
    const int k = total_nodes_in_P;
    const bool outside_gap_ok =
        g_enable_outside_gap && outside_gap_holds(deltaP);

    if (g_enable_structural_child) {
        const bool a_singleton = p_bucket_singleton(deltaP);
        const bool b_empty = p_bucket_empty(deltaP + 1);
        if (a_singleton) {
            num_structural_A_singleton_nodes++;
            if (b_empty) num_structural_A_singleton_B_empty_nodes++;
        }
    }

    // children_stack[k] is our sibling-iteration buffer at this depth.
    // Recursive calls only touch children_stack[k+1] and deeper, so it is stable
    // across the sibling loop at the bottom of this function.
    std::vector<int>& children_vec = children_stack[k];
    children_vec.clear();

    const bool maintain_pool_here = g_enable_uncle_inh && (k >= cui_pool_start_depth);
    if (maintain_pool_here) {
        // === CUI path: maintain full D_psi(K) in dpsi_stack[k] ===
        std::vector<int>& my_pool = dpsi_stack[k];
        my_pool.clear();

        // Gate: inherit from parent's D_psi iff theta*k >= delta(P) + 1
        // Also require parent depth to have pool maintenance enabled.
        const bool have_parent_pool = (k >= 2) && ((k - 1) >= cui_pool_start_depth);
        const bool uncle_ok =
            have_parent_pool &&
            (static_cast<double>(theta) * static_cast<double>(k)
                >= static_cast<double>(deltaP) + 1.0 - 1e-12);

        // Heuristic veto: prefer inheritance only when parent pool is not
        // much larger than the local narrow-scan workload estimate.
        const size_t parent_pool_size = have_parent_pool ? dpsi_stack[k - 1].size() : 0u;
        const size_t narrow_scan_cost =
            (deltaP >= 1 ? static_cast<size_t>(countNP[deltaP - 1]) : 0u) +
            static_cast<size_t>(countNP[deltaP]) +
            (deltaP + 1 < static_cast<int>(countNP.size()) ? static_cast<size_t>(countNP[deltaP + 1]) : 0u);
        const size_t inherit_budget = kUncleInheritVetoFactor * std::max<size_t>(1u, narrow_scan_cost);
        const bool inherit_worth_it = parent_pool_size <= inherit_budget;

        if (uncle_ok && inherit_worth_it) {
            const std::vector<int>& parent_pool = dpsi_stack[k - 1];
            for (int u : parent_pool) {
                if (tracks[u].inP) continue;
                if (static_cast<int>(tracks[u].degNP) >= min_add_deg) {
                    my_pool.push_back(u);
                }
            }
            numcalls_saved_by_uncle_inh++;
        } else {
            collect_full_dpsi_from_np(min_add_deg, my_pool);
        }

#if DENSE_PCE_CHECK_DPSI
        debug_assert_dpsi_matches_current(min_add_deg, my_pool);
#endif

        // Maximality: empty D_psi(K) <=> no vertex can extend K.
        if (my_pool.empty()) {
            if (k >= min_size) report_if_new(k);
            return;
        }

        if (outside_gap_ok) {
            // Outside-Gap Corollary:
            // if maxNPdeg < deltaP, then D_psi(K) = Children(K).
            // my_pool is already the exact D_psi(K), so no Lemma-5 filtering is needed.
            children_vec = my_pool;
            num_outside_gap_fastpath_hits++;
            num_outside_gap_candidates +=
                static_cast<unsigned long long>(children_vec.size());
        } else if (uncle_ok && inherit_worth_it) {
            // Inherited path: derive children from D_psi(K) by Lemma 5.
            for (int u : my_pool) {
                if (is_child_by_lemma5_ii_iii(u, v, deltaP)) {
                    children_vec.push_back(u);
                }
            }
        } else {
            // Fallback path in CUI mode: keep full pool for descendants,
            // but use local narrow scan to build immediate children.
            collect_children_by_narrow_scan(v, min_add_deg, deltaP, children_vec);
        }
    } else {
        // === Non-CUI path: original narrow bucket scan (modes 1-4) ===
        // Bit-for-bit equivalent to the r1-m1 baseline, no dpsi_stack maintenance.
        if (k >= 0 && k < static_cast<int>(dpsi_stack.size())) dpsi_stack[k].clear();
    ...
    if (g_enable_order_bound && order_ub > 0 && total_nodes_in_P == order_ub) {
        return;
    }

    if (children_vec.size() > 1) std::sort(children_vec.begin(), children_vec.end());

    // Iterate by index: add/remove does not mutate children_stack[k], and the
    // recursive call writes only to depth k+1.
    for (size_t i = 0; i < children_vec.size(); ++i) {
        const int u = children_vec[i];
        add_to_inside_P(u);
...
int main(int argc, char* argv[]) {
    ...
    for (; arg_idx < argc; ++arg_idx) {
        ...
        if (arg == "--theta" && arg_idx + 1 < argc) {
            ...
            if (mode == 1) {
                g_enable_order_bound = false; g_enable_edge_bound = false; g_enable_turan = false; g_enable_uncle_inh = false; g_enable_outside_gap = false; g_enable_structural_child = false;
            } else if (mode == 2) {
                g_enable_order_bound = true; g_enable_edge_bound = false; g_enable_turan = false; g_enable_uncle_inh = false; g_enable_outside_gap = false; g_enable_structural_child = false;
            } else if (mode == 3) {
                g_enable_order_bound = true; g_enable_edge_bound = true; g_enable_turan = false; g_enable_uncle_inh = false; g_enable_outside_gap = false; g_enable_structural_child = false;
            } else if (mode == 4) {
                g_enable_order_bound = true; g_enable_edge_bound = true; g_enable_turan = true; g_enable_uncle_inh = false; g_enable_outside_gap = false; g_enable_structural_child = false;
            } else if (mode == 5) {
                g_enable_order_bound = true; g_enable_edge_bound = true; g_enable_turan = true; g_enable_uncle_inh = true; g_enable_outside_gap = false; g_enable_structural_child = false;
            } else if (mode == 6) {
                g_enable_order_bound = true; g_enable_edge_bound = true; g_enable_turan = true; g_enable_uncle_inh = true; g_enable_outside_gap = true; g_enable_structural_child = false;
            } else if (mode == 7) {
                g_enable_order_bound = true; g_enable_edge_bound = true; g_enable_turan = true; g_enable_uncle_inh = true; g_enable_outside_gap = false; g_enable_structural_child = true;
            } else if (mode == 8) {
                g_enable_order_bound = true; g_enable_edge_bound = true; g_enable_turan = true; g_enable_uncle_inh = true; g_enable_outside_gap = true; g_enable_structural_child = true;
            } else {
                std::cerr << "Error: --mode must be one of {1,2,3,4,5,6,7,8}." << std::endl;
                return 1;
            }
            ...
            g_enable_uncle_inh = false;
        } else if (arg == "--no-outside-gap") {
            g_enable_outside_gap = false;
        } else if (arg == "--no-structural-child") {
            g_enable_structural_child = false;
        } else if (arg == "--order") {
            ...
            g_enable_uncle_inh = true;
        } else if (arg == "--outside-gap") {
            g_enable_outside_gap = true;
        } else if (arg == "--structural-child") {
            g_enable_structural_child = true;
        } else {
    ...
    std::cout << "Gates => order:" << (g_enable_order_bound?"on":"off")
              ...
              << " uncle_inh:" << (g_enable_uncle_inh?"on":"off")
              << " outside_gap:" << (g_enable_outside_gap?"on":"off")
              << " structural_child:" << (g_enable_structural_child?"on":"off") << std::endl;
    ...
    std::cout << "Uncle-inheritance saves: " << PC.get_numcalls_saved_by_uncle_inh() << std::endl;
    std::cout << "Outside-gap fastpath hits: " << PC.get_num_outside_gap_fastpath_hits() << std::endl;
    std::cout << "Outside-gap candidates accepted directly: "
              << PC.get_num_outside_gap_candidates() << std::endl;
    std::cout << "Structural A-singleton nodes: "
              << PC.get_num_structural_A_singleton_nodes() << std::endl;
    std::cout << "Structural A-singleton & B-empty nodes: "
              << PC.get_num_structural_A_singleton_B_empty_nodes() << std::endl;
    std::cout << "Structural Type-2 fastpath hits: "
              << PC.get_num_structural_type2_fastpath_hits() << std::endl;
    std::cout << "Structural Type-3 partial fastpath uses: "
              << PC.get_num_structural_type3_partial_uses() << std::endl;
    std::cout << "Structural Type-3 full fastpath hits: "
              << PC.get_num_structural_type3_full_hits() << std::endl;
    std::cout << "Structural total candidate hits: "
              << PC.get_num_structural_total_candidate_hits() << std::endl;
