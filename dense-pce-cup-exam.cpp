...
static bool g_enable_uncle_pruning = true; // CLI only; CUP enumeration removed from this translation unit
...
void PseudoCliqueEnumerator::iter(int v) {
    ...
    // Reverse-search children: Lemma 5 bucket scan only (parent-feasible pool / CUP enumeration removed;
    // see --no-uncle / --uncle flags kept for CLI compatibility; they do not change enumeration).
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0]
                  ...
                  << "  [--no-uncle | --uncle]  (legacy; enumeration is Lemma-5 buckets only in this build)"
                  << std::endl;
        return 1;
    }
    ...
    std::cout << "Gates => order:" << (g_enable_order_bound?"on":"off")
              ...
              << " uncle-flag:" << (g_enable_uncle_pruning?"on":"off") << " (ignored; search uses Lemma-5 buckets only)"
              << std::endl;
    ...
    const int total_iter = PC.get_iter_count();
    const unsigned long long edge_prunes = PC.get_numcalls_saved_by_edge_bound();
    std::cout << "\nTotal Iterations: " << total_iter << std::endl;
    std::cout << "Edge-bound prunes saved: " << edge_prunes;
    if (total_iter > 0) {
        std::cout << "  (prunes/iter = " << (static_cast<long double>(edge_prunes) / static_cast<long double>(total_iter)) << ")";
    }
    std::cout << std::endl;
