...
public:
    ...
    // Load graph from BBkC binaries and build BOTH:
    ...
    void read_graph_from_bbkcbinaries(const std::string& dir, const std::string& original_graph_path) {
        ...
        // (A) EBBkC CSR: exact copy of the binary layout (order preserved)
        csr_n = n; csr_m = m;
        csr_node_off.assign(n+1, 0);
        for (uint32_t i=0;i<n;++i) csr_node_off[i+1] = csr_node_off[i] + degrees[i];
        if (csr_node_off[n] != m) {
            std::cerr << "Warning: degree prefix sum != m (" << csr_node_off[n] << " vs " << m << ")\n";
        ...
        // Stream neighbors once; fill both CSRs, then sort each row slice of our CSR
        for (uint32_t u=0; u<n; ++u) {
            const uint32_t du  = degrees[u];
            const uint32_t off = csr_node_off[u];
            ...
            for (uint32_t k=0; k<du; ++k) {
                ...
                csr_edge_dst[off + k]  = static_cast<int>(v);  // EBBkC view
                csr_neighbors[off + k] = v;                    // our view (will sort)
            }
...
public:
    PseudoCliqueEnumerator(Graph& graph, float theta = 1.0, int min_size = 1, int max_size = -1, int order_ub = -1)
    : graph(graph), theta(theta), min_size(min_size) {
        
        this->order_ub = order_ub;
    ...
    // Enumerate starting from a Turán seed (R-clique) provided by EBBkC
    void enumerate_with_turan(const std::vector<int>& turan_seed) {
        ...
        std::sort(sorted_seed.begin(), sorted_seed.end(),
                [&](int a, int b) {
                    // degree proxy from CSR slice lengths
                    const uint32_t da = graph.csr_offsets[a+1] - graph.csr_offsets[a];
        ...
        int deltaP = 0;
        for (int d = 0; d <= total_nodes_in_P; ++d) {
            if (d >= 0 && d < (int)headP.size() && headP[d] != -1) { deltaP = d; break; }
        }
        int v_star = -1;
...
private:
    ...
    // By default: no global de-dup set — MD ordering + Type-2/3 tie-breaks prevent duplicates.
    #ifndef PCE_VALIDATE_DEDUP
    ...
    #else
    // Optional lightweight validator (compile with -DPCE_VALIDATE_DEDUP only in debug runs)
...
void PseudoCliqueEnumerator::add_vertex_internal(int v) {
    ...
    uint32_t begin = (v >= 0 && v + 1 < (int)graph.csr_offsets.size())
                     ? graph.csr_offsets[(size_t)v] : 0u;
    uint32_t end   = (v >= 0 && v + 1 < (int)graph.csr_offsets.size())
                     ? graph.csr_offsets[(size_t)v + 1] : begin;
...
void PseudoCliqueEnumerator::remove_from_inside_P(int v) {
        ...
        if (tracks[adj_u].inP) {
                total_edges_in_P--;
        int degree = static_cast<int>(tracks[adj_u].degP);
...
void PseudoCliqueEnumerator::iter(int v) {
    ...
    // EDGE bound: only when R ≤ |P| < ℓ  (match mod-edge-order/fpce)
    if (total_nodes_in_P >= seed_R && total_nodes_in_P < min_size) {
    ...
    // Heuristic reservation based on size of degree bucket around δ(P)
    {
        int hint = 0;
        if (deltaP >= 0 && deltaP < (int)headNP.size()) {
            hint = (deltaP < (int)countNP.size()) ? countNP[deltaP] : 0;
            if (deltaP - 1 >= 0 && deltaP - 1 < (int)countNP.size()) hint += countNP[deltaP - 1];
            if (deltaP + 1 < (int)countNP.size()) hint += countNP[deltaP + 1];
        }
    ...
    for (int u = headNP[deltaP]; u != -1; u = nextNP[u]) {
        ...
        if (u < v) {
            ...
            bool valid = true;
            // adjacency to all x in P[δ(P)] with x < u
            for (int x = headP[deltaP]; x != -1; x = nextP[x]) {
                if (x < u && !graph.has_edge(u, x)) { valid = false; break; }
            }
            if (valid) { children_vec.push_back(u); ++c; }
        }
    }

    // ---- Type 3: degree == δ(P)+1 ----
    if (deltaP + 1 < static_cast<int>(headNP.size())) {
        for (int u = headNP[deltaP + 1]; u != -1; u = nextNP[u]) {
            if (tracks[u].inP || tracks[u].degNP < min_add_deg) continue;
            if (!graph.has_edge(v, u) || !(v > u)) continue; // tie-break to v*

            bool valid = true;
            // (1) adjacency to all x in P[δ(P)]
            for (int x = headP[deltaP]; x != -1; x = nextP[x]) {
                if (!graph.has_edge(u, x)) { valid = false; break; }
            }
            // (2) adjacency to prefix of P[δ(P)+1]: only x < u
            if (valid) {
                int targetDeg = deltaP + 1;
    ...
    // Enforce deterministic child processing order (lexicographic by vertex id)
