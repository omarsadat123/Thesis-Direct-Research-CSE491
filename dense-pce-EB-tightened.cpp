...
// Deterministic pruning: NP degree-sum upper bound feasibility-to-ℓ
static bool g_enable_npdeg_ub    = true;
// Tightening for NPdeg UB: bound internal edges among added vertices using degeneracy (ξ_G)
static bool g_npdeg_use_degeneracy_internal_ub = true;
...
private:
    ...
    unsigned long long numcalls_saved_by_npdeg_ub   = 0ULL;
    ...
    // Upper bound on #edges in an induced subgraph of size t in a d-degenerate graph.
    // Fact: any d-degenerate graph on n vertices has at most d*n - d(d+1)/2 edges (for n >= d+1),
    // and always at most C(n,2). Using global d=ξ_G is safe (may be loose but never invalid).
    inline long long ub_internal_edges_by_degeneracy(int t) const {
        if (t <= 1) return 0LL;
        const long long clique_ub = 1LL * t * (t - 1) / 2;
        if (!g_npdeg_use_degeneracy_internal_ub) return clique_ub;

        const int d = graph.degeneracy;
        if (d <= 0) return 0LL;

        // If d >= t-1, degeneracy doesn't tighten beyond clique bound.
        if (d >= t - 1) return clique_ub;

        // d*t - d(d+1)/2 (tight simple closed-form UB for d-degenerate graphs)
        long long degen_ub = 1LL * d * t - 1LL * d * (d + 1) / 2;
        if (degen_ub < 0) degen_ub = 0;
        return std::min(clique_ub, degen_ub);
    }

    // Deterministic feasibility pruning (drop-in):
    // Upper bound on edges achievable when extending current P to size ℓ by adding
    // t = ℓ - |P| outside vertices with the largest current degNP (edges into P),
    // and an UB on edges among the added vertices (clique UB or degeneracy UB).
    // If even this optimistic UB cannot reach the required LHS edges for size ℓ,
    // then this branch can never produce any (ℓ,θ)-pseudo-clique.
    bool prune_by_npdeg_ub() const {
        const int S = min_size;                 // ℓ
        const int P = total_nodes_in_P;         // |P|
        const int t = S - P;                    // vertices still needed to reach ℓ

        if (t <= 0) return false;               // already at/above ℓ
        // Not enough vertices remaining in the whole graph to reach ℓ
        if (graph.total_nodes - P < t) return true;

        long long sum_top_degNP = 0LL;
        int rem = t;
        // Scan degree histogram from high to low. countNP stores OUTSIDE-P vertices only.
        for (int deg = maxNPdeg; deg > 0 && rem > 0; --deg) {
            if (deg >= (int)countNP.size()) continue;
            const int cnt = countNP[deg];
            if (cnt <= 0) continue;
            const int take = (cnt < rem) ? cnt : rem;
            sum_top_degNP += 1LL * take * deg;
            rem -= take;
        }
        // Any remaining vertices (if rem>0) must be degree-0 into P, contributing 0.

        const long long ub_internal = ub_internal_edges_by_degeneracy(t);
        const long long ub_edges =
            1LL * total_edges_in_P +
            sum_top_degNP +
            ub_internal;

        return ub_edges < 1LL * lhs_required_edges;
    }
...
public:
    ...
    unsigned long long get_numcalls_saved_by_npdeg_ub() const {
        return numcalls_saved_by_npdeg_ub;
    }
...
void PseudoCliqueEnumerator::iter(int v) {
    ...
    // NP degree-sum UB: apply only when |P| < ℓ.
    // Cheap, correctness-preserving feasibility test for reaching the required
    // edge count at size ℓ, using the current degNP histogram (outside-P degrees into P).
    if (g_enable_npdeg_ub && total_nodes_in_P < min_size) {
        if (prune_by_npdeg_ub()) {
            numcalls_saved_by_npdeg_ub++;
            return;
        }
    }
    ...
    for (int u = headNP[deltaP]; u != -1; u = nextNP[u]) {
        ...
        if (u < v) {
            ...
            for (int x = headP[deltaP]; x != -1; x = nextP[x]) {
                if (x < u && !graph.has_edge(u, x)) { valid = false; break; } // if the neighbor is not connected to any other node x in the minimum-degree bucket that is smaller than u, skip
            }
            if (valid) children_vec.push_back(u); // add the neighbor to the children vector
        }
    }
    ...
    // ---- Type 3: deg == δ(P)+1 ----
    {
        ...
        if (targetDeg < static_cast<int>(headNP.size())) {
            for (int u = headNP[targetDeg]; u != -1; u = nextNP[u]) {
                ...
                for (int x = headP[deltaP]; x != -1; x = nextP[x]) { // iterate through the neighbors of vertex v with degree deltaP
                    if (!graph.has_edge(u, x)) { valid = false; break; } // if the neighbor is not connected to any other node x in the minimum-degree bucket that is smaller than u, skip
                }
                ...
                if (valid && targetDeg < static_cast<int>(headP.size())) {
                    for (int x = headP[targetDeg]; x != -1; x = nextP[x]) { // iterate through the neighbors of vertex v with degree targetDeg
                        if (x < u && !graph.has_edge(u, x)) { valid = false; break; } // if the neighbor is not connected to any other node x in the (deltaP + 1) bucket that is smaller than u, skip
                    }
                }
...
int main(int argc, char* argv[]) {
    ...
    for (; arg_idx < argc; ++arg_idx) {
        ...
        if (arg == "--theta" && arg_idx + 1 < argc) {
            ...
            if (mode == 1) {
                g_enable_order_bound = false; g_enable_edge_bound = false; g_enable_turan = false; g_enable_npdeg_ub = false;
            } else if (mode == 2) {
                g_enable_order_bound = true;  g_enable_edge_bound = false; g_enable_turan = false; g_enable_npdeg_ub = false;
            } else if (mode == 3) {
                g_enable_order_bound = true;  g_enable_edge_bound = true;  g_enable_turan = false; g_enable_npdeg_ub = true;
            } else {
                g_enable_order_bound = true;  g_enable_edge_bound = true;  g_enable_turan = true;  g_enable_npdeg_ub = true;
            }
        ...
        } else if (arg == "--no-npdeg") {
            g_enable_npdeg_ub = false;
        } else if (arg == "--npdeg-clique-only") {
            // Disables degeneracy-tightened internal UB; falls back to C(t,2)
            g_npdeg_use_degeneracy_internal_ub = false;
        } else if (arg == "--order") {
            ...
            g_enable_npdeg_ub = true;
            g_npdeg_use_degeneracy_internal_ub = true;
        } else {
    ...
    std::cout << "Gates => order:" << (g_enable_order_bound?"on":"off")
              ...
              << " npdeg:" << (g_enable_npdeg_ub?"on":"off")
    ...
    std::cout << "NPdeg-UB prunes saved: " << PC.get_numcalls_saved_by_npdeg_ub() << std::endl;
