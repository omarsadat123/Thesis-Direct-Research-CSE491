...
// Defaults: full FPCE path (order+edge+Turan+SO-EB)
...
private:
    ...
    // SO-EB: conservative Lemma-6 pre-filter evaluated at parent P,
    // before paying add_to_inside_P cost for candidate u.
    // deltaP = min_degree_in_P_fast(), already computed in iter().
    inline bool so_edge_bound_prune(int u, int deltaP) const {
        ...
        const double rhs = static_cast<double>(e_hat)
                         + t_prime * (delta_hat + (t_prime + 1) * 0.5);
...
public:
    ...
    unsigned long long get_numcalls_saved_by_so_edge_bound() const {
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Gates => order:" << (g_enable_order_bound?"on":"off")
              ...
              << " soeb:" << (g_enable_so_edge_bound?"on":"off") << std::endl;
